-- Timetable assignments schema (FINAL CORRECTED VERSION)
-- This table stores the actual timetable assignments linking subjects, teachers, classrooms, and time slots
-- Uses UUID foreign keys to match existing database structure
-- Fixed RLS policies with proper type casting

-- Create timetable_assignments table with UUID foreign keys
...
-- Allow admins to insert timetable assignments (with proper type casting)
...
-- Allow admins to update timetable assignments (with proper type casting)
...
-- Allow admins to delete timetable assignments (with proper type casting)
...
-- Note: Adjust column names based on your actual time_slots table structure
...
SELECT 
    ...
    -- Time slot details (using original column names from corrected_schema.sql)
