...
-- Users table (using Firebase Auth IDs as primary keys)
CREATE TABLE users (
...
CREATE TABLE teachers (
  ...
  user_id TEXT REFERENCES users(id) ON DELETE CASCADE, -- Firebase Auth UID
  name VARCHAR(255) NOT NULL,
  email VARCHAR(255) UNIQUE NOT NULL,
...
CREATE TABLE timetables (
  ...
  created_by TEXT REFERENCES users(id) ON DELETE SET NULL, -- Firebase Auth UID
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- RLS Policies for users table (FIXED - no UUID casting)
...
-- Allow admins and teachers to insert teacher records
CREATE POLICY "Allow teacher creation" ON teachers
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM users 
      WHERE users.id = auth.uid() 
      AND (users.role = 'admin' OR users.id = user_id)
    )
  );
